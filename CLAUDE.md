...
- `agents/` `commands/` `skills/` `hooks/` `CLAUDE.md` はリポジトリ直下に置き、`bin/install.sh` で `~/.claude/` 配下に symlink する。リポジトリ側の編集が直接 `~/.claude/` に反映される
...
- 「Planner→Generator→Evaluator」のような複数ファイルにまたがるエージェント・フローは **plugin として `plugins/<name>/` に閉じる**。コマンドや agent は `/<plugin>:<command>` `<plugin>:<agent>` の形で自動 namespace 化されるため、ルートが汚れない
- リポジトリ自身が一人用 marketplace（`.claude-plugin/marketplace.json`）。新しい系を増やすときは `plugins/<name>/` を追加して `marketplace.json` の `plugins` 配列に1エントリ足すだけ
...
2. フックや専用の権限が必要なら `hooks/hooks.json` と `settings.json` を plugin 配下に置く（ルートの `settings.json` には足さない）
3. `.claude-plugin/marketplace.json` の `plugins` 配列に1行追加
