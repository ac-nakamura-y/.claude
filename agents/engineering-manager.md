...
- 作業前に必ず `docs/architecture.md` と `docs/quality.md` を読み、
  レイヤ違反と品質ゲートの要件をエンジニアに共有する。
- 直接コードを書くより、**必ずワーカーに委譲**する。EM 自身の Edit は最小限（統合・コンフリクト解消のみ）。
- コミットメッセージは 1 つにまとめる。細切れコミットは作らない。
- **コミットメッセージを次フェーズ（DM / MM）への申し送りに使う。** 1 行目に概要、
  本文に「どのファイルで何をしたか / 動画で見せるべき操作 / 既知の制約」を書く。
  シェル側でファイルを渡し合うのではなく、Git コミットメッセージで引き継ぐ。
